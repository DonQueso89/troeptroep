...
const SignUpForm = ({ event }) => {
  ...
  return (
    <form action={formUrl} method="post" autoComplete="true" onSubmit={formSubmitter}>
      ...
      <div className={actionStyles.signUpFormInput}>
        <label htmlFor="gear">I need some gear</label>
        <input
          type="checkbox"
          name="needs_gear"
          id="needs_gear"
          value={needsGear}
          onChange={(e) => setNeedsGear(e.target.value)}
        />
        <div className={actionStyles.gearInfo} >
          <FaQuestionCircle />
        </div>
          <div className={actionStyles.gearTooltip}>
            <p>If you dont have any gear, we can bring a ring, grabber and gloves for you.</p>
          </div>
      </div>
      <div className={actionStyles.signUpFormInput}>
